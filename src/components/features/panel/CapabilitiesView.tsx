...
export const CapabilitiesView: FC<CapabilitiesViewProps> = ({
    ...
    return (
        <div className="h-full flex flex-col space-y-4">
            ...
            <div className={`rounded-2xl sm:rounded-3xl bg-white dark:bg-gradient-to-r dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 p-3 sm:p-5 shadow-xl border-2 ${activeCatData?.borderColor || 'border-gray-200'} relative overflow-hidden transition-all duration-500`}>
                {/* Decorative background elements - themed */}
                <div className={`absolute -right-4 -top-4 w-16 h-16 sm:w-24 sm:h-24 ${activeCatData?.bgLight || 'bg-blue-500/10'} rounded-full blur-2xl`}></div>
                <div className={`absolute -left-4 -bottom-4 w-12 h-12 sm:w-20 sm:h-20 ${activeCatData?.bgLight || 'bg-blue-500/10'} rounded-full blur-xl`}></div>
                ...
                <div className="relative z-10 flex flex-row-reverse items-center justify-between">
                    <div className="flex flex-col items-end">
                        ...
                        <div className="flex items-baseline gap-1.5">
                            <span className={`text-4xl sm:text-6xl font-black bg-clip-text text-transparent bg-gradient-to-br ${activeCategory === activeCatData?.id ? 'from-gray-900 to-gray-600 dark:from-white dark:to-gray-300' : 'from-blue-600 to-blue-400'}`}>
                                {Math.round(score)}
                            </span>
                            <span className="text-lg sm:text-xl text-gray-400 font-bold">/ 100</span>
                        </div>
                    ...
                    <div className="text-left">
                        ...
                        <div className="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white">{poiList.length}</div>
                        <div className="text-[10px] sm:text-[11px] text-gray-400 font-bold uppercase tracking-widest">Nokta</div>
                    </div>
                </div>
